...
export default async ({vm, value = null}) => {
  ...
  if (complaintPhenomenonLevel) {
    ...
    const {res, err} = await (process.env.VUE_APP_TEMPLATE_FORM_IS_MOCK === 'true' ? vm.$$api.template.formMock : vm.$$api.complaint.formPhenomenon)({
    ...
    if (res?.formContent) {
      ...
      if (vm.formData.formContentMd5 && (vm.formData.formContentMd5 === formContentMd5)) return;//记录是否与上次模板相同 则不替换
      ...
      vm.formData.formContentMd5 = formContentMd5 || null;
  ...
  vm.formData.formContentMd5 = null;//没有则清除
  vm.removeAppendItems(['complaint_scene_form']);//失败
}
