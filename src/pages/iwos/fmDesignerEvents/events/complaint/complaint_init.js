...
export default async ({vm, item}) => {
  ...
  if (!vm.$route.params.workorderId) { // 新建
    ...
    // 工单画像 // 省内接口_设备产品信息查询
    const {res, err} = R1, {res: dciRes, err: dciErr} = R2;
  ...
  if (res) {
    ...
    await vm.expandFormConfigItems.find(efci => efci.key === 'complaint$dot$complaintPhenomenonLevel')?.onChange({
      vm,
