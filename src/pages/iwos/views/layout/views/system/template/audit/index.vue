...
const getList = async (pageNum = pageInfo.value.pageNum) => {
  ...
  const {res, err} = await proxy.$$api.template.pendingApprovalList({params: Object.assign(proxy.$$formatELDateTimeRange(formData.timeRange, ['startTime', 'endTime']), pageInfo.value, formData, {sceneCode: sceneCode})});
  if (err) return;
...
const formConfigItems = ref([
  ...
  {col: 6, type: 'divider-empty'}, {col: 6, type: 'divider-empty'}, {col: 6, type: 'divider-empty', isShow: () => proxy.$store.getters['user/GET_USER_PROVINCE_CODE'] !== '8100000'},
  {
