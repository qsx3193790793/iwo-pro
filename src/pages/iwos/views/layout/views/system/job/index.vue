    ...
    <el-dialog title="任务详情" :visible.sync="openView" width="700px" append-to-body>
      <el-form ref="form" :model="form" label-position="left" label-width="120px" size="small">
        <el-row>
          ...
          <el-col :span="12">
            <el-form-item label="任务分组：">{{ $store.getters['dictionaries/MATCH_LABEL']('sys_job_group', form.jobGroup) }}</el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="下次执行时间：">{{ $$dateFormatterYMDHMS(form.nextValidTime) }}</el-form-item>
          </el-col>
          <el-col :span="24">
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      this.$$api.job.listJob({params: this.queryParams}).then(({res: response, err}) => {
    ...
    handleStatusChange(row) {
      ...
      this.$$Dialog.confirm('确认要"' + text + '""' + row.jobName + '"任务吗？').then(() => {
        return this.$$api.job.changeJobStatus({data: {jobId: row.jobId, status: row.status}});
      }).then(({res, err}) => {
        if (err) return;
    ...
    /* 立即执行一次 */
    handleRun(row) {
      this.$$Dialog.confirm('确认要立即执行一次"' + row.jobName + '"任务吗？').then(() => {
        return this.$$api.job.runJob({data: {jobId: row.jobId, jobGroup: row.jobGroup}});
      }).then(({res, err}) => {
        if (err) return;
    ...
    /** 任务详细信息 */
    handleView(row) {
      this.$$api.job.getJob({jobId: row.jobId}).then(({res, err}) => {
        if (err) return;
        this.form = res;
        this.openView = true;
      });
    ...
    /** 任务日志列表查询 */
    handleJobLog(row) {
      this.$router.push({name: 'JobLog', params: {jobId: row.jobId || 0}})
    },
    /** 新增按钮操作 */
    handleAdd() {
      this.reset();
    ...
    handleUpdate(row) {
      ...
      this.$$api.job.getJob({jobId}).then(({res, err}) => {
        if (err) return;
        this.form = res;
        this.open = true;
        this.title = "修改任务";
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.jobId != undefined) {
            this.$$api.job.updateJob({data: this.form}).then(({res, err}) => {
              if (err) return;
            ...
            this.$$api.job.addJob({data: this.form}).then(({res, err}) => {
              if (err) return;
    ...
    handleDelete(row) {
      ...
      this.$$Dialog.confirm('是否确认删除定时任务编号为"' + jobIds + '"的数据项？').then(() => {
        return this.$$api.job.delJob({jobId: jobIds});
      }).then(({res, err}) => {
        if (err) return;
        this.getList();
