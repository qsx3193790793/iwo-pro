...
function resolveMonacoPath(filePath) {
  try {
    return require.resolve(path.join('src/components/MonacoEditor/esm', filePath));
  } catch (err) {
    try {
      return require.resolve(path.join(process.cwd(), 'src/components/MonacoEditor/esm', filePath));
    } catch (err) {
